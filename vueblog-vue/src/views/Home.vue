<template>
  <div class="home-container">
    <el-row :gutter="60">
      <el-col :span="12" v-for="blog in blogs" :key="blog.id">
        <el-card
        ...
        >
          <div class="home-title">
            ...
            <span style="font-size: small; color: blue"
              >创建时间：{{ blog.createTime.split(" ")[0] }}
            </span>
            <span style="font-size: small; color: blue">
              更新时间：{{ blog.updateTime.split(" ")[0] }}
            </span>
            <span style="font-size: small; color: blue">
              分类：{{ blog.typeName }}</span
            >
            <span style="font-size: small; color: blue">
              字数：{{ blog.words }}</span
            >
...
export default {
  ...
  methods: {
    ...
    getData(currentPage) {
      ...
      const _this = this;
      this.$axios.get("/blogs?pageNum=" + currentPage).then((res) => {
        ...
        for (const i in _this.blogs) {
          for (const j in _this.types) {
